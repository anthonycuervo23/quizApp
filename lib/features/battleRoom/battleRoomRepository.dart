...
import 'package:quizappuic/features/battleRoom/battleRoomRemoteDataSource.dart';
...
class BattleRoomRepository {
  ...
  Future<Map<String, dynamic>> joinMultiUserBattleRoom(
    ...
    try {
      ...
      QuerySnapshot querySnapshot =
          await _battleRoomRemoteDataSource.getMultiUserBattleRoom(roomCode);
  ...
  Future<List<Question>> getQuestions(
      ...
      required String roomDocumentId}) async {
    try {
      List? quesions;
      if (forMultiUser) {
        ...
        quesions = await _battleRoomRemoteDataSource.getQuestions(
            categoryId: categoryId, languageId: languageId, matchId: matchId);
      }
  ...
  Future<void> startMultiUserQuiz(String? battleRoomDocumentId) async {
    try {
      _battleRoomRemoteDataSource
          .updateMultiUserRoom(battleRoomDocumentId, {"readyToPlay": true});
    } catch (e) {}
  }

  //All the message related code start from here
  Stream<Message> subscribeToOpponentMessages(
      {required String roomId, required String by}) {
    return _battleRoomRemoteDataSource
        .subscribeToOpponentMessages(roomId: roomId, by: by)
        .map((event) {
